...
class SparseExplainer(object):
    ...
    def explain(self, inp, ind=None):
        batch_size, n_chs, img_witdh, img_height = inp.shape
        delta = torch.zeros((batch_size, n_chs, img_witdh * img_height)).cuda()
        ...
        for i in range(self.n_iterations):
            ...
            inp_grad = inp_grad.view((batch_size, n_chs, img_witdh * img_height))
            hessian_delta_vp, = torch.autograd.grad(
                    inp_grad.dot(delta).sum(), inp, create_graph=True)
            hessian_delta_vp = hessian_delta_vp.view((batch_size, n_chs, img_witdh * img_height))
            taylor_1 = inp_grad.dot(delta)
            taylor_2 = 0.5 * delta.dot(hessian_delta_vp)
            taylor_1 = taylor_1.sum()  # sum of batch
            taylor_2 = taylor_2.sum()  # sum of batch
            ...
            loss.backward()
            optimizer.step()
        delta = delta.view((batch_size, n_chs, img_witdh, img_height))
        return delta.data
