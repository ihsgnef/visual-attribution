...
class NoiseAttack(object):
    ...
    def get_noisy(self, inp):        
        ...
        return Variable(torch.from_numpy(perturbed_inp).float().cuda(), requires_grad = True)    
...
def main():
    ...
    for model_name, method_name, viz_style, kwargs in model_methods:
        ...
        saliency_org = get_saliency(model, explainer, deepcopy(inp_org), raw_img,
        ...
        saliency_gho = get_saliency(model, explainer, deepcopy(noisy_inp.data), raw_img,
                                    model_name, method_name, viz_style,
                                    filename_g)
